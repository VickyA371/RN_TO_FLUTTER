...
class RNFlutterView: UIView, HostCounterApi {
  ...
  @objc var onJsonChange: RCTDirectEventBlock?
  @objc var onScreenChange: RCTDirectEventBlock?

  @objc var clicks: NSNumber? {
    didSet {
      if let c = clicks {
        flutterCounterApi.setClicks(value: c.int64Value) {_ in }
      }
    }
  }
  @objc var text: String? {
    didSet {
      if let t = text {
        flutterCounterApi.setText(text: t) {_ in }
      }
    }
  }
  @objc var json: String? {
    didSet {
      if let t = json {
        print("JSON : \(json)")
        flutterCounterApi.setJson(json: t) {_ in }
      }
    }
  }
  @objc var screen: String? {
    didSet {
      if let s = screen {
        flutterCounterApi.setScreen(screen: s) {_ in }
      }
    }
  }
  @objc var theme: String? {
    didSet {
      if let t = theme {
        flutterCounterApi.setTheme(theme: t) {_ in }
      }
    }
  }
  ...
  func setJson(json: String) throws {
    print("New JSON : \(json)")
    onJsonChange?(["json": json, "target": self.reactTag ?? NSNull()])
  }
