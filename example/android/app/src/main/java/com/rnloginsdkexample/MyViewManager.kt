...
class MyViewManager(
  ...
  @ReactProp(name = "json")
  fun setJson(view: ViewGroup, value: String) {
    when (val viewState = fragmentMap[view.id]) {
      null -> fragmentMap[view.id] = FlutterViewState(
        initialJson = value,
      )
      else -> {
        if (viewState.initialJson == null) {
          viewState.initialJson = value
        }
        if (viewState.fragment != null) {
          viewState.fragment!!.flutterCounterApi?.setJson(value){}
        }
      }
    }
  }
...
//        view.layout(view.left, view.top, view.right, view.bottom)
  ...
  override fun getExportedCustomDirectEventTypeConstants(): Map<String, Map<String, String>> {
    return MapBuilder.of(
      ...
      COMMAND_JSON,
      MapBuilder.of("registrationName", "onJsonChange"),
    )
  }
  ...
  companion object {
    ...
    const val COMMAND_JSON = "setJson"
  }
}
